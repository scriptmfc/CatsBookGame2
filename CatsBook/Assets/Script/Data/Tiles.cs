...
public class Tiles : MonoBehaviour {
    ...
    void Start()
    {
        GameManagerTemp.Instance.tiles = new Tile[width, length];
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                GameManagerTemp.Instance.tiles[x,y] = GenerateTile(x, y);
                
                if(x >= GameManagerTemp.Instance.floor1_length || y>= GameManagerTemp.Instance.floor1_width)
                {
                    GameManagerTemp.Instance.tiles[x, y].enable = false;
                }
                else
                {
                    GameManagerTemp.Instance.tiles[x, y].enable = true;
                }
    ...
    public Tile GetTileByCoordinate(int x, int y)
    {
        ...
        return GameManagerTemp.Instance.tiles[x, y];
    }
